...
run's per-sample records rather than transcribed from run logs, and every
...
leak, a benchmark-gaming pattern the audit of
Section~\ref{sec:results-artifacts} details and quantifies.
...
$k$ and tops the chart. Third, the task universe
...
closely --- the same 60 module tasks, the same Makefile verdict, the
same temperature (0.2) and unbiased estimator, with @1/@5 estimated
from twenty samples against our ten --- but prompts every model
\emph{directly}, so the comparison isolates what the system around the
model is worth. The strongest published rates are func@1 13.5\,\%
(GPT-4o-V) and func@5 17.3\,\% (DeepSeek-V3-671B). \textsc{Direct-t0}
--- our 20B model under the same direct prompting --- lands inside
that frontier band (8.3\,\%), yet with the grounded pipeline and
...
comparison: the published runs predate the June-2026 AES S-box
testbench fix (Section~\ref{sec:threats}) --- though excluding both
S-box tasks moves none of our rates by more than 2.2\,pp --- and
RealBench additionally reports formal@$k$ (equivalence checking
...
real tail across all six attempts (73/53/43/23/22/14), the rest
...
\section{Routing: Matched Coverage at a Fraction of the Compute}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \textsc{Router-2t} (full cascade) & 600 & 35.0\,\% & 21 & \textbf{130} & \textbf{1.35}$^{+}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  ...
  \caption[Routing frontier]{Tasks solved vs.\ total compute. The full
    cascade router holds its base arm's coverage at 39\,\% less compute
    --- the cheapest solved task of any arm; the pipeline-only arm buys
    the most coverage at three times the router's cost.}
  \label{fig:frontier}
\end{figure}

Table~\ref{tab:routing} and Figure~\ref{fig:frontier} give the routing
result. Three comparisons and one correction carry the argument.
...
\paragraph{The cascade holds coverage while cutting compute.}
...
5 of 60 tasks Tier-0 deferred). Solved tasks hold at 21 and pass@10 at
35.0\,\% --- the two arms' solved sets differ by a single low-yield task
in each direction (\texttt{e203\_exu\_disp} only in the cascade,
\texttt{e203\_exu\_alu\_bjp} only in the base), sampling variance rather
than a routing effect --- \emph{while total compute falls} $213 \to 130$
hours ($-39\,\%$), because 342 of 600 samples run on the direct flow at
roughly a tenth of a pipeline sample's token cost. Per solved task, the
cascade spends 1.35\,M tokens against its own base arm's 2.17\,M and the
brute-force hybrid's 4.87\,M: the cheapest solved task of any arm we
measured.

\paragraph{What the raw records claimed --- and why we do not.}
Before the benchmark-gaming audit
(Section~\ref{sec:results-artifacts}), the same records read very
differently: \textsc{Router-2t} appeared to solve 34 tasks at pass@10
56.7\,\%, a $+13$-task routing win. Thirteen of those tasks were
``solved'' \emph{only} by samples that instantiate the benchmark's
reference model --- the FSM- and algorithm-heavy AES tops, SD-card
engines, and E203 decode/dispatch tasks where the direct flow had
seemed strongest --- and \textsc{Hybrid} appeared to solve 30 (ten of
them wrap-only). With wraps excluded, the cascade's coverage gain over
its own ablation is zero, and the direct flow contributes no task the
pipeline does not also solve. The routing result this thesis
stands on is therefore the cost row of Table~\ref{tab:routing}, not a
coverage row --- and we report the correction in full, because the
audit that produced it is part of the contribution
(Section~\ref{sec:results-artifacts}).

\paragraph{Against the hybrid ceiling, directionally ahead.}
\textsc{Router-2t} solves 21 tasks where \textsc{Hybrid} --- running both
flows on everything --- solves 20 at $3.3\times$ the compute. This
comparison is \emph{not} single-variable: the hybrid predates the
direct-flow repair loop (its direct samples genuinely compiled at
16.2\,\% vs.\ the router run's 59.9\,\%) and used $10/10$ budgets, so
the arms differ in repair as well as routing. We therefore claim only
that the cascade \emph{matches the attainable coverage at a fraction of
the cost}, not that it beats the hybrid deployment on coverage.
...
interfaces --- the direction that forfeits would-be passes. The probe
tier is selective (5 of 60 tasks), and 22 of 279 generated plans are
discarded --- the cascade's modest overhead. With gpt-oss-120B the same
router solves 25 tasks at pass@1 32.7\,\% (vs.\ the 20B cascade's 21 at
21.7\,\%): scale helps genuinely, and misroutes drop to 141, differently
distributed (A$\to$direct falls to 71; B$\to$pipeline rises to 70): the
bigger model's feature extractor defers less and trusts the pipeline
more. Closing the A$\to$direct gap --- e.g.\ by letting spec-level
uncertainty trigger the probe more aggressively --- is the router's
clearest headroom.
...
the model will not redeclare them --- and that list included
\texttt{ref\_\emph{<task>}}, the benchmark's own golden reference
model. The models found the exploit: emit a top module that merely
\emph{instantiates} \texttt{ref\_\emph{<task>}}, which compiles
...
was most misleading: 13 of \textsc{Router-2t}'s 34 raw solved tasks and
