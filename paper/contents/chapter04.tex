...
an order of magnitude more tokens per sample. Its planning
...
is precisely where the plan pays. The residual algorithmic tasks
...
of the thesis. A per-task \emph{cascade router} predicts, before
...
One caution applies to the whole chapter. The benchmark-gaming audit
(Section~\ref{sec:results-artifacts}) later shows that the solution
set this rule's polarity was fitted on was contaminated, so
Section~\ref{sec:results-routing} evaluates both the rule as designed
here and a version re-derived from the cleaned evidence.
...
sample's token cost. On exactly those tasks the pipeline's
...
(\texttt{e203\_ifu\_minidec}). These are tasks whose difficulty is wiring
...
\texttt{e203\_extend\_csr}, \texttt{e203\_irq\_sync}). These are modules whose
behavior fits in one coherent piece and offers nothing to reuse.
...
accuracy can be scored. The audit of
Section~\ref{sec:results-artifacts} later invalidates the target-flow
mapping the labels carry (post-cleaning, only 4 of the 28 class-B
tasks are solved by the direct flow alone), so
Section~\ref{sec:results-routing} scores routing correctness against
empirical flow-solvability instead and flags the label table for
recomputation.
...
The uncertain remainder is not noise; it is a coherent third type of
...
magnitude of compute reduction, provided the class assignment is real.
Section~\ref{sec:results-routing} tests that premise against the cleaned
evidence.
...
pipeline flow leans on the loop harder, as expected for the giant integrators
routed to it. The functional stage converts 130 samples into
...
\begin{figure}[htbp]
  ...
  \caption[Sample flow through the cascade]{How the full-cascade run's
    ...
    from the specification; the plan probe resolves the 5 uncertain
...
task). After the audit invalidates the labels' target-flow mapping,
Section~\ref{sec:results-routing} replaces the label-based confusion
matrix with correctness against \emph{empirical flow-solvability} ---
whether each task solvable by exactly one flow was routed to that
flow. The design goal, stated once and measured throughout, is to hold the
