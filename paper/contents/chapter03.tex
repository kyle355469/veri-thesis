...
are habitually used --- and the way that habit fails on tasks of this
...
it} --- competing objectives that confuse one another and strain the
context window, with the interface details that matter most buried in
the middle of it. Separating the work into a planning stage (decide the
...
about: accuracy, because each stage sees the right information at the
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/thesis-full-architecture.png}
  \caption[Complete system architecture]{Complete architecture of the
    generation system. The specification is parsed into sub-specs and
    chunk-summarized into a merged generation plan (bottom center); the
    generator LLM produces RTL inside a Verilator/Yosys syntax repair
    loop backed by the semantic repair cache, followed by
    testbench-driven functional repair (top); interface and behavior
    slices of the specification feed the two repair loops. The key-attribute
    extraction path on the left is the cascade router of
    Chapter~\ref{ch:04}, which decides per task whether generation
...
\begin{enumerate}
  \item \textbf{Catalog construction.} The dependency sources that the
    ...
    per-task \emph{reusable-IP catalog}: one entry per provided module,
  ...
  \item \textbf{Planning.} The planner (Section~\ref{sec:pipeline-planner})
    ...
    plan: requirements, module decomposition, a reuse decision per submodule
  ...
  \item \textbf{Plan adaptation.} An adapter normalizes the plan, resolves
    ...
    generation prompt: the original specification, the signatures of reused
    modules, environment notes, and the testbench's DUT instantiation
...
\texttt{new\_rtl\_required} is the plan's load-bearing output, and its
two error directions cost very differently. Declaring reuse of a module
that does not fit (or does not exist under that name) plants a fatal
seed downstream: the generator either re-implements a shipped dependency
...
and \texttt{new\_rtl\_required} otherwise, with the grounding mechanisms
below guaranteeing that whichever path produced the name, it resolves to
...
new-RTL modules as behavioral requirements to implement.
...
\begin{enumerate}
  \item \textbf{Catalog injection.} The real catalog vocabulary --- every
    ...
    vocabulary: \texttt{selected\_ip} must be one of the listed ids.
  \item \textbf{Closed-vocabulary grounding.} Every \texttt{selected\_ip} in
    ...
    the submodule marked \texttt{new\_rtl\_required}. The plan additionally
...
the model re-declared anyway are stripped before verification.
...
the benchmark's syntax verdict on 60 of 60 tasks. The cost of this fidelity
...
Three mechanisms sit on top of the generator's inner loop. Throughout the
thesis, a repair \emph{budget} ``$s/f$'' means at most $s$ syntax-repair and
$f$ functional-repair attempts per sample.
...
cache} (Figure~\ref{fig:semantic-diff}) that reuses the model's own past
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/thesis-semantic-diff.png}
  \caption[Semantic repair cache]{The semantic repair cache. Verified
    diagnostic$\to$fix pairs from the model's own repairs are keyed by
    identifier-stripped diagnostic signatures and replayed as advisory
    hints when similar failures recur.}
...
not change it --- and the loop keeps the best \emph{compiling} candidate by
lowest mismatch count, never advancing to a candidate that regresses
syntax.
...
slicer extracts those identifiers, then assembles a focused slice: the
...
together with the repair success rate: a smaller prompt that dropped the
fixing sentence would be a pure regression. The budget calibration of
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
      ...
      box/.style={draw, rounded corners=2pt, align=center, inner sep=5pt,
                  minimum height=8mm},
      ctx/.style={draw, rounded corners=2pt, align=center, inner sep=4pt,
                  minimum height=7mm, fill=black!4},
      gate/.style={draw, rounded corners=2pt, align=center, inner sep=5pt,
                   minimum height=8mm, very thick},
      arr/.style={-{Stealth[length=2.5mm]}},
    ]
    \node[ctx] (plan) {adapted plan\\+ reused-IP sources};
    \node[ctx, below=4mm of plan] (iface) {interface slice\\+ DUT port contract};
    \node[ctx, below=4mm of iface] (behav) {behavior slice};
    \node[box, right=12mm of iface] (llm) {generator /\\repair LLM};
    \node[box, right=12mm of llm] (rtl) {RTL\\candidate};
    \node[gate, above right=2mm and 12mm of rtl] (lint) {Verilator lint\\(benchmark file set)};
    \node[gate, below right=2mm and 12mm of rtl] (tb) {testbench\\simulation};
    \node[box, above=9mm of llm] (cache) {semantic repair cache\\(verified diagnostic$\to$diff hints)};
    \node[box, right=10mm of lint, align=center] (out) {final\\candidate};
    \draw[arr] (plan) -| ([xshift=-6mm]llm.west) -- (llm.west);
    \draw[arr] (iface) -- (llm);
    \draw[arr] (behav) -| ([xshift=-6mm]llm.west);
    \draw[arr] (llm) -- (rtl);
    \draw[arr] (rtl.east) -- ++(4mm,0) |- (lint.west);
    \draw[arr] (lint.south) |- node[below, pos=0.75, font=\footnotesize]
      {diagnostics (syntax loop, $\leq s$)} ([yshift=2mm]llm.north east);
    \draw[arr] (lint.east) -- node[above, font=\footnotesize]{clean} (out);
    \draw[arr] ([xshift=6mm]rtl.east) |- (tb.west);
    \draw[arr] (tb.south) -- ++(0,-4mm) -| node[below, pos=0.25, font=\footnotesize]
      {mismatch report (functional loop, $\leq f$)} (llm.south);
    \draw[arr] (cache) -- node[right, font=\footnotesize]{hints} (llm);
    \draw[arr] (lint.north) |- node[above, pos=0.75, font=\footnotesize]
      {verified fixes} (cache.east);
  \end{tikzpicture}
  \caption[Verify-and-repair loop]{The generator's verify-and-repair loop.
    A candidate first iterates in the syntax repair loop against the
    benchmark-faithful Verilator lint (budget $s$), with the semantic
    repair cache injecting verified diagnostic$\to$fix hints and
    absorbing new ones; compiling candidates then enter the functional
    repair loop against the testbench (budget $f$), which keeps the best
    compiling candidate by mismatch count. Interface and behavior slices
    of the specification target each loop's prompt.}
  \label{fig:repair-loop}
\end{figure}

\section{Summary}

The pipeline's design distills to three commitments. \emph{Ground every
decision}: the planner works over the per-task catalog through its
reuse tools, and every reuse decision it emits is additionally resolved
...
fatal contracts visible during repair rather than at scoring, and the
