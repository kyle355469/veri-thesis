...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[box, right=10mm of iface] (llm) {generator /\\repair LLM};
    \node[box, right=10mm of llm] (rtl) {RTL\\candidate};
    \node[gate, above right=2mm and 10mm of rtl] (lint) {Verilator lint\\(benchmark file set)};
    \node[gate, below right=2mm and 10mm of rtl] (tb) {testbench\\simulation};
    \node[box, above=14mm of llm, xshift=9mm] (cache) {semantic repair cache\\(verified diagnostic$\to$diff hints)};
    ...
    \draw[arr] (lint.south) |- node[above, pos=0.83, align=center, font=\footnotesize]
      {diagnostics\\(syntax loop, $\leq s$)} ([yshift=3mm]llm.north east);
    ...
    \draw[arr] (cache.south -| llm.north) -- node[right, font=\footnotesize]{hints} (llm.north);
